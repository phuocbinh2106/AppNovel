...
class AccountFragment : Fragment() {
    ...
    private fun listenCoins() {
        ...
        val userId = try {
            ...
            // Nếu bị lỗi ClassCast (do ID cũ là Int), hãy xóa đi để user đăng nhập lại
            sharedPref.edit().remove("userId").apply()
            null
        }

        if (userId.isNullOrEmpty()) {
            android.util.Log.e("FirebaseCoin", "UserId is null or empty, cannot listen for coins")
            return
        }
        ...
        firestoreListener = db.collection("users")
            ...
            .addSnapshotListener { snap, error ->
                if (error != null) {
                    android.util.Log.e("FirebaseCoin", "Listen failed: ${error.message}")
                ...
                if (snap != null && snap.exists()) {
                    ...
                    android.util.Log.e("FirebaseCoin", "Document does not exist for ID: $userId")
                }
            }
    }
